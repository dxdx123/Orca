...
public static class UQtAlgo
{
    ...
    public static void BuildRecursively(UQtNode node, UQtConfig config)
    {
        // parameters
        float subWidth = node.Bound.width * 0.5f;
        float subHeight = node.Bound.height * 0.5f;
        bool isPartible = subWidth >= config.CellSizeThreshold && subHeight >= config.CellSizeThreshold;
        ...
        out List<UQtLeaf> inLeaves, out List<UQtLeaf> outLeaves, UQtConfig config)
    {
        ...
        GenerateLeavesByDist(node, active, config.CellSwapOutDist, ref outList);
