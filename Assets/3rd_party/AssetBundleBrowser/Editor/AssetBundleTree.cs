...
namespace AssetBundleBrowser
{
    ...
    { 
        ...
        {
            // asset path --> assetBundle path
            // Dictionary<string, string> dict = new Dictionary<string, string>();
            Dictionary<string, string> dict = AssetBundleBuildTools.GetAssetBundleDict();

            var bundleDict = ParseBundleDict(dict);

            foreach (var item in bundleDict)
            {
                string bundleName = item.Key.Replace(".", "_");
                List<string> list = item.Value;

                var newBundle = AssetBundleModel.Model.CreateEmptyBundle(null, bundleName);
                foreach (var srcPath in list)
                {
                    AssetBundleModel.Model.MoveAssetToBundle(srcPath, newBundle.m_Name.bundleName, newBundle.m_Name.variant);
                }
            }
        ...
        private Dictionary<string, List<string>> ParseBundleDict(Dictionary<string,string> dict)
        {
            ...
            foreach (var item in dict)
            {
                ...
                string destAB = item.Value;

                List<string> list;
                if (bundleDict.TryGetValue(destAB, out list))
                {
                    list.Add(srcPath);
                }
                else
